...
def neighbors_setup(cols,
                    rows):  # sätter upp hur många minor som varje cell har intill sig (om cellen inte själv är en mina)
...
def Play_Time(Name):
    ...
    if cols == 16 and rows == 30 and mines == 99:
        ...
        text = 'Name:' + Name + ' Rows:' + str(rows) + ' Cols:' + str(cols) + ' Mines:' + str(mines) + ' Time:' + str(
            Time) + 's' + '\n'
        file.write(text)


def Win(
        idx):  # En funktion som ska ge ett meddelande till spelaren när den har vunnit, genom att alla minor är flaggade eller alla icke minor är röjda
...
def Lose(
        idx):  # en funktion som kallas när spelaren har klickat på en mina som signalerar att spelaren har förlorat samt avslöjar spelbrädet
    global Lose_window
    Lose_window = Tk()
    Lose_window.title('')
    WindowHeight = 50
